...
%           resultado.totalIt: # total de iterações
%           resultado.pop_it: matriz contendo as posições de cada partícula
%                             em cada uma das iterações (cada par de vetor
%                             colunas corresponde a uma iteração)
%           resultado.x_it: matriz contendo a melhor posição (solução) de
%                             cada iteração (cada linha corresponde a uma iteração)
%           resultado.x:   vetor linha de dimensão 2, corresponde a melhor
%                             solução encontrada pelo algoritmo
%
%   fun: um function_handle contendo a função a ser plotada:
%        exemplo:
%                   fun=@(a)-((a(1)-1)^2+(a(2)-1)^2);
%
%  options: estrutura contendo as opções
%           options.limites: [lmin lmax] -> limites mínimo e máximo do
%                            gráfico
%           
